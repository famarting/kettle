...
public class ApiServerRequestHandler implements RequestHandler {
    ...
    /**
     * Cluster-scoped resources: GET /apis/GROUP/VERSION/RESOURCETYPE - return the
     * collection of resources of the resource type GET
     * /apis/GROUP/VERSION/RESOURCETYPE/NAME - return the resource with NAME under
     * the resource type Namespace-scoped resources: GET
     * /apis/GROUP/VERSION/RESOURCETYPE - return the collection of all instances of
     * the resource type across all namespaces GET
     * /apis/GROUP/VERSION/namespaces/NAMESPACE/RESOURCETYPE - return collection of
     * all instances of the resource type in NAMESPACE GET
     * /apis/GROUP/VERSION/namespaces/NAMESPACE/RESOURCETYPE/NAME - return the
     * instance of the resource type with NAME in NAMESPACE
     */

    protected ApiServerRequestContext validateRequest(RoutingContext ctx) throws RequestValidationException {
        String path = ctx.request().path();
        String[] pathItems = Stream.of(path.split("/")).filter(s -> s != null && !s.trim().isEmpty())
                .toArray(String[]::new);
        ...
        if ( ((apiType == ApiType.API_SERVICE && group == null && definition.getGroup() == null)
                || (apiType == ApiType.API_GROUP && group.equals(definition.getGroup())))
                && version.equals(definition.getVersion()) && type.scope() == definition.getScope() ) {
            return new ApiServerRequestContext(definition.getGroup(), definition.getVersion(),
                    definition.getNames().getKind(), ctx, type, name);
        } else {
    ...
    protected void handlePut(ApiServerRequestContext requestContext)
    ...
    protected void handlePatch(ApiServerRequestContext requestContext)
            throws JsonParseException, JsonMappingException, IOException, RequestValidationException {
        if ( requestContext.resourceName().isPresent() ) {
            // Resource resource = validateRequestBody(requestContext);
    ...
    protected void handlePost(ApiServerRequestContext requestContext)
            throws JsonParseException, JsonMappingException, IOException, RequestValidationException {
        Resource resource = validateRequestBody(requestContext);
    ...
    private Resource validateRequestBody(ApiServerRequestContext requestContext)
    ...
    private void sendResponse(ApiServerRequestContext requestContext, Object resource) throws JsonProcessingException {
        if ( Optional.ofNullable(requestContext.httpContext().request().getHeader(HttpHeaders.ACCEPT)).orElse("")
                .contains("application/json;as=Table;v=v1beta1;g=meta.k8s.io") ) {
        ...
        String responseContentType = Optional
                .ofNullable(Optional.ofNullable(requestContext.httpContext().request().getHeader(HttpHeaders.ACCEPT))
                        .map(acceptHeader -> acceptHeader != null && acceptHeader.equals("*/*") ? null : acceptHeader)
                        .map(acceptHeader -> {
                            return acceptHeader.contains("*/*")
                                    ? acceptHeader.replace("*/*", "").trim().replace(",", "")
                                    : acceptHeader;
        ...
        requestContext.httpContext().response().putHeader(HttpHeaders.CONTENT_TYPE, responseContentType)
                .end(responseBody);
    }
